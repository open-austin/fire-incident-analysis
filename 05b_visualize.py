...
from census_variables import URBAN_CLASS
...
def load_data():
    ...
    tracts = gpd.read_file("processed_data/census_tracts_with_incidents.geojson")
    print(f"  Census tracts: {len(tracts)}")
    
    # Load Census Demographic data
    print("  Loading census tract summary data...")
    demo = pd.read_csv("processed_data/census_demographics.csv", dtype={'tract_code': str})
    demo = demo[demo['median_income'] > 0]  # Filter out invalid income data

    # Load incident data
    print("  Loading incident data...")
    incidents = pd.read_csv("processed_data/incidents_exploded_by_tract.csv", dtype={'tract_code': str})
    

    # Aggregate to calendar year for display purposes
    incidents_yr = incidents.groupby(['tract_code', 'calendaryear']).agg({'incident_number': 'count'}).reset_index()
    incidents_yr.rename(columns={'incident_number': 'total_incidents'}, inplace=True)
    
    years = [2022, 2023, 2024]
    year_stg = pd.DataFrame({
        'calendaryear': years
    })
    tracts = tracts.merge(year_stg, how='cross')

    # Merge the incident yearly data
    print("  Merging tract shapes with incident data...")
    tracts = tracts.merge(incidents_yr, on=['tract_code', 'calendaryear'], how='left')
    tracts.fillna({'total_incidents': 0}, inplace=True)

    # Merge the demographic data
    print("  Merging tract shapes with incident data...")
    tracts = tracts.merge(demo, on='tract_code', how='inner')

    # Check match rate
    matched = tracts['total_incidents'].notna().sum()
    print(f"    Tracts with incident data: {matched} ({matched/len(tracts)*100:.1f}%)")
...
def create_choropleth_map(gdf, column, title, filename, colormap='YlOrRd', scale='linear'):
    ...
    tooltip = folium.GeoJsonTooltip(
        fields=['GEOID', column, 'population', 'urban_class'],
        aliases=['Census Tract:', f'{title}:', 'Population:', 'Classification:'],
        localize=True
    )
...
def create_categorical_map(gdf, column, title, filename, colors=None):
    ...
    # Add GeoJson layer
    tooltip = folium.GeoJsonTooltip(
        fields=['tract_code', column, 'population', 'incidents_per_1000_pop'],
        aliases=['Census Tract:', 'Classification:', 'Population:', 'Incidents/1000:'],
        localize=True
    )
...
def create_building_age_chart_yearly(filename):
    ...
    gdf = gpd.read_file("processed_data/census_tracts_with_incidents.geojson")
    ...
    gdf['age_class'] = gdf['pct_built_2010_plus'].apply(
    ...
    pop_by_age = gdf.groupby('age_class')['population'].sum()
    ...
    for yi, (year, year_color, year_label) in enumerate(zip(years, year_colors, year_labels)):
        ...
        for ac in age_classes:
            year_ac_data = gdf[(gdf['calendaryear'] == year) & (gdf['age_class'] == ac)]
...
def create_urban_comparison_yearly(filename):
    ...
    pop_by_class = gdf[gdf['urban_class'].isin(URBAN_CLASS.values())].groupby('urban_class')['population'].sum()
    ...
    x = range(len(URBAN_CLASS.values()))
    width = 0.25

    for yi, (year, year_color, year_label) in enumerate(zip(years, year_colors, year_labels)):
        rates = []
        for uc in URBAN_CLASS.values():
            year_uc_data = gdf[(gdf['calendaryear'] == year) & (gdf['urban_class'] == uc)]
...
def create_building_age_map(gdf, filename):
    ...
    folium.Choropleth(
        geo_data=valid_gdf.__geo_interface__,
        data=valid_gdf,
        columns=['tract_code', 'pct_built_2010_plus'],
        key_on='feature.properties.tract_code',
    ...
    tooltip = folium.GeoJsonTooltip(
        fields=['tract_code', 'pct_built_2010_plus', 'pct_built_pre1970', 'incidents_per_1000_pop'],
        aliases=['Census Tract:', '% Built 2010+:', '% Built pre-1970:', 'Incidents/1000:'],
        localize=True
    )
...
def create_station_map(gdf, filename):
    ...
    valid_gdf = gdf[gdf['population_density'].notna() & (gdf['population_density'] > 0)].copy()

    folium.Choropleth(
        geo_data=valid_gdf.__geo_interface__,
        data=valid_gdf,
        columns=['tract_code', 'population_density'],
        key_on='feature.properties.tract_code',
        fill_color='YlOrRd',
        fill_opacity=0.5,
        line_opacity=0.2,
...
def main():
    ...
    # Load data
    tracts, summary_urban, summary_housing = load_data()
    ...
    create_scatter_plot(tracts, 'chart_housing_correlation.png')
    ...
    print("\nCreating new maps...")
    if HAS_FOLIUM:
        create_building_age_map(tracts, 'map_building_age.html')
        create_station_map(tracts, 'map_fire_stations.html')
